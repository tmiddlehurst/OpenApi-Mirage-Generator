...
import { getObjectProperties, getRandomStringNumberOrBool, getValueForProperty, writeFactoryFile, writeModelDefinitionsFile } from '../src/writeFiles';
...
const exampleFactory = "  import { Factory } from 'miragejs';\n\n      export default Factory.extend(\n        { valid:true,modifiedDate:null,nameOnCard:\"Thomas Middlehurst\",totalAmountDeposited:3747,scheme:\"VISA\",transactions:\"instrumentalist\",\"date\",\"coinsurance\",\"speed\" }\n      )\n ";

// test('writes model definitions file from modelNames', async () => {
//   const modelNames = ['Member', 'PaymentCard'];
//   const expectedOutput = await prettier.format(exampleModels, {
//     semi: true, singleQuote: true, parser: 'typescript', trailingComma: 'none'
//   });
//   const input = await writeModelDefinitionsFile(modelNames);
...
test('writes a factory file for a simple model', async () => {
  ...
  const res = await writeFactoryFile(modelName, schema);
